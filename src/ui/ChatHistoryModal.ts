...
export class ChatHistoryModal extends Modal {
    ...
    async onOpen(): Promise<void> {
        ...
        if (this.conversations.length === 0) {
            this.contentEl.createEl('p', {
                ...
                text: 'No saved conversations yet. Conversations are saved automatically after each task.',
            });
            return;
        }
    ...
    private renderRow(container: HTMLElement, conv: SavedConversation): void {
        ...
        info.createDiv('chat-history-title').setText(conv.title || 'Untitled');
        ...
        const loadBtn = actions.createEl('button', { cls: 'mod-cta chat-history-load-btn', text: 'Open' });
